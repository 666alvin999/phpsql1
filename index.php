    ...
    <body>
        ...
        <section id="registered-pokemon">
            ...
            if (array_key_exists("type", $_GET)) {
                $pokemons = json_decode(file_get_contents("http://localhost/pokebuild/getPokemon.php?type=" . $_GET['type']));
            } elseif (array_key_exists("gen", $_GET)) {
                $pokemons = json_decode(file_get_contents("http://localhost/pokebuild/getPokemon.php?gen=" . $_GET['gen']));
            } else {
                $pokemons = json_decode(file_get_contents("http://localhost/pokebuild/getPokemon.php"));
            }
            ...
            <div id="type-filter">
                <?php
                $knownTypes = json_decode(file_get_contents("http://localhost/pokebuild/getTypes.php"))
                ?>

                <?php foreach ($knownTypes as $type) : ?>
                    <a href="http://localhost/pokebuild/index.php?type=<?= $type->typeName ?>">
                            ...
                            <div class="pre-evolution">
                                <?php if (gettype($pokemon->preEvolution) == "object") : ?>
                                    <h3 class="pre-evolution-name">
                                        ...
                                        <a href="http://localhost/pokebuild/pokemon.php?name=<?= $pokemon->preEvolution->name ?>">
