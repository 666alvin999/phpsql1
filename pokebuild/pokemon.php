...
<?php
    require "menu.php";
    $pokemon = json_decode(file_get_contents("http://localhost/phpsql1/pokebuild/getPokemon.php?name=" . $_GET['name']));
...
<div id="pokemon-card">
    <div id="main-frame">
        <div class="pokemon-name">
            <h2>
                <a href="http://localhost/phpsql1/pokebuild/pokemon.php?name=<?= $pokemon->name ?>">
                    <?= $pokemon->name ?>
                </a>
                #<?= $pokemon->id ?>
            </h2>
...
<div id="related-pokemons">
    <?php if (gettype($pokemon->preEvolution) != "string") : ?>
        <div class="related">
            <?php
            $preEvolution = json_decode(file_get_contents("http://localhost/phpsql1/pokebuild/getPokemon.php?name=" . $pokemon->preEvolution->name));
            ?>
            ...
            <h3 class="related-pokemon-name">
                <a href="http://localhost/phpsql1/pokebuild/pokemon.php?name=<?= $preEvolution->name ?>">
                    <?= $preEvolution->name ?>
                </a>
            </h3>
        </div>
    ...
    <?php foreach ($pokemon->evolutions as $evolution) : ?>
        <div class="related">
            <?php
            $evolution = json_decode(file_get_contents("http://localhost/phpsql1/pokebuild/getPokemon.php?name=" . $evolution->name));
            ?>
            ...
            <h3 class="related-pokemon-name">
                <a href="http://localhost/phpsql1/pokebuild/pokemon.php?name=<?= $evolution->name ?>">
                    <?= $evolution->name ?>
                </a>
            </h3>
        </div>
